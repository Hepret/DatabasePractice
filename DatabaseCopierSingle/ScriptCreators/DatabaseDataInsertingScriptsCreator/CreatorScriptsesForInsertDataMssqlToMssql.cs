...
namespace DatabaseCopierSingle.ScriptCreators.DatabaseDataInsertingScriptsCreator
{
    ...
    private string CreateItemString(object item, SchemaColumn columnInfo)
    {
        ...
        switch (item)
        {
            ...
            case DateTime date when columnInfo.Data_type.ToLower() == "datetime2" ||
                                    columnInfo.Data_type.ToLower() == "datetime":
                return $"'" +
                       $"{date.Year}-{date.Month}-{date.Day} " +
                       $"{date.Hour}:{date.Minute}:{date.Second}:{date.Millisecond}'";
            case DateTimeOffset date when columnInfo.Data_type == "datetimeoffset":
                return $"'" +
                       $"{date.Year}-{date.Month}-{date.Day} " +
